...
live_active = on_command(
    "live_on", rule=bot_is_active, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER
)
live_deactivate = on_command(
    "live_off", rule=bot_is_active, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER
)

dyn_active = on_command(
    "dyn_on", rule=bot_is_active, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER
)
dyn_deactivate = on_command(
    "dyn_off", rule=bot_is_active, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER
)
...
async def _():
    await bot_help.finish(
        MessageSegment.image(
            "https://i0.hdslb.com/bfs/new_dyn/abef945ad1d209ad1d2360624180a15d490040351.png"
        )
    )
